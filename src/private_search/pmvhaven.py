...
def is_pmvhaven_url(url: str) -> bool:
    ...
    return host == "pmvhaven.com" and VIDEO_PATH_PATTERN.search(path) is not None and VIDEO_ID_PATTERN.search(path) is not None
...
def fetch_metadata(url: str, session: requests.Session | None = None) -> PMVHavenMetadata:
