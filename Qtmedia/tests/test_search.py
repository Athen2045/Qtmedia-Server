...
from qtmedia.search.engine import (
    ADAPTERS,
    SearchCandidate,
    SiteAdapter,
    VideoResult,
...
from qtmedia.search.quality import relevance_score as quality_relevance_score
from qtmedia.search.quality import term_matches, tokenize
...
def test_impersonate_for_url_uses_the_hosting_adapters_profile():
    # SpankBang's current upstream workaround is the bare Chrome target,
    # rather than a fixed Chrome version or the stale Safari profile.
    assert impersonate_for_url("https://spankbang.com/abcde/video/some-title") == "chrome"
...
def test_ydl_options_does_not_apply_global_impersonation_by_default(monkeypatch):
    calls = []
    monkeypatch.setattr(
        search_module.http_client,
...
def test_ydl_options_uses_opt_in_firefox_cookies_for_spankbang(monkeypatch):
    monkeypatch.setenv("PRIVATE_SEARCH_CLI_YTDLP_COOKIES_FROM_BROWSER", "firefox")
    monkeypatch.setenv("PRIVATE_SEARCH_CLI_YTDLP_USER_AGENT", "Firefox/142.0")

    options = search_module.ydl_options(
        "chrome", "https://spankbang.com/abcde/video/title"
    )

    assert options["cookiesfrombrowser"] == ("firefox", None, None, None)
    assert options["cachedir"] is False
    assert options["http_headers"]["User-Agent"] == "Firefox/142.0"
